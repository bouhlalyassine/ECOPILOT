...
export function WaterForm({ sites }: WaterFormProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch('/api/modules/water', {
      method: 'POST',
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="flex flex-col text-sm">
          ...
          <input
            ...
            step="0.01"
            value={form.eau_m3}
            onChange={(event) => handleChange('eau_m3', Number(event.target.value))}
            className="mt-1 rounded border border-gray-300 px-2 py-1"
          />
        </label>
      </div>
