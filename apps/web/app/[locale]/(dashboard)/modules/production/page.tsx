...
export default async function ProductionModulePage({ params }: { params: { locale: string } }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <ProductionForm sites={sites.map((site) => ({ id: site.id, name: site.name }))} />
        ...
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2 text-left">Surface (ha)</th>
              <th className="px-4 py-2 text-left">Production (kg)</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {records.map((record) => (
              <tr key={record.id}>
                ...
                <td className="px-4 py-2">{Number(record.sup_ha).toFixed(2)}</td>
                <td className="px-4 py-2">{Number(record.prod_kg).toFixed(2)}</td>
