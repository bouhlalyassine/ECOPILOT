...
const productionSchema = z.object({
  ...
  familleCulture: z.string().min(1),
  variete: z.string().min(1),
  sup_ha: z.number().nonnegative(),
  prod_kg: z.number().nonnegative(),
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = productionSchema.parse(body);
    ...
    const record = await prisma.productionRecord.create({
      data: {
        ...
        familleCulture: parsed.familleCulture,
        variete: parsed.variete,
        sup_ha: parsed.sup_ha,
        prod_kg: parsed.prod_kg,
