...
const updateSchema = z.object({
  ...
  sup_ha: z.number().optional(),
  prod_kg: z.number().optional()
});
...
export async function PATCH(
  ...
  try {
    ...
    const record = await prisma.productionRecord.findUnique({ where: { id: params.id } });
    if (!record) {
    ...
    const updated = await prisma.productionRecord.update({
      where: { id: params.id },
      data: {
        sup_ha: parsed.sup_ha ?? record.sup_ha,
        prod_kg: parsed.prod_kg ?? record.prod_kg,
