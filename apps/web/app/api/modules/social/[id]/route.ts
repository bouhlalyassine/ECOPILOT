...
export async function PATCH(
  ...
  try {
    ...
    const record = await prisma.socialAction.findUnique({ where: { id: params.id } });
    if (!record) {
    ...
    const updated = await prisma.socialAction.update({
      where: { id: params.id },
      data: {
        budget: parsed.budget ?? record.budget,
        beneficiaries: parsed.beneficiaries ?? record.beneficiaries,
